 ...
 */
lv_group_t *lv_test_group_1(lv_group_t *g1)
{
    lv_coord_t hres = lv_disp_get_hor_res(NULL);
    lv_coord_t vres = lv_disp_get_ver_res(NULL);

    //g = lv_group_create();
    if (g1) lv_group_set_focus_cb(g1, group_focus_cb);
    ...
    if (g1) lv_indev_set_group(win_kb_indev, g1);
    ...
    if (g) lv_indev_set_group(real_kb_indev, g);
#endif
    ...
    if (g) lv_indev_set_group(enc_indev, g);
#endif
    ...
    if (g1) lv_group_add_obj(g1, lv_win_get_content(win));
    ...
    if (g) lv_group_add_obj(g, obj);
#endif
    obj = lv_btn_create(win, NULL);
    if (g1) lv_group_add_obj(g1, obj);
