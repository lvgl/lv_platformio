...
static uint32_t tft_fb[LV_HOR_RES * LV_VER_RES];
...
void monitor_flush(int32_t x1, int32_t y1, int32_t x2, int32_t y2, const lv_color_t * color_p)
{
    ...
    for(y = y1; y <= y2; y++) {
        for(x = x1; x <= x2; x++) {
            tft_fb[y * LV_HOR_RES + x] = lv_color_to32(*color_p);
            color_p++;
        }
    ...
    uint32_t w = x2 - x1 + 1;
    for(y = y1; y <= y2; y++) {
        memcpy(&tft_fb[y * LV__HOR_RES + x1], color_p, w * sizeof(lv_color_t));
...
void monitor_fill(int32_t x1, int32_t y1, int32_t x2, int32_t y2, lv_color_t color)
{
    ...
    for(x = act_x1; x <= act_x2; x++) {
        for(y = act_y1; y <= act_y2; y++) {
            ...
            tft_fb[y * LV_HOR_RES + x] = color32;
#else
            tft_fb_act[y * LV_HOR_RES + x] = color32;
#endif
        }
    }
...
void monitor_map(int32_t x1, int32_t y1, int32_t x2, int32_t y2, const lv_color_t * color_p)
{
    ...
    for(y = act_y1; y <= act_y2; y++) {
        for(x = act_x1; x <= act_x2; x++) {
            ...
            tft_fb[y * LV_HOR_RES + x] = lv_color_to32(*color_p);
#else
            tft_fb_act[y * LV_HOR_RES + x] = lv_color_to32(*color_p);
#endif
            color_p++;
        }
...
static void monitor_sdl_init(void)
{
    ...
    window = SDL_CreateWindow("TFT Simulator",
                              ...
                              LV_HOR_RES * MONITOR_ZOOM, LV_VER_RES * MONITOR_ZOOM, 0);       /*last param. SDL_WINDOW_BORDERLESS to hide borders*/
    ...
    texture = SDL_CreateTexture(renderer,
                                SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STATIC, LV_HOR_RES, LV_VER_RES);
    SDL_SetTextureBlendMode(texture, SDL_BLENDMODE_BLEND);

    /*Initialize the frame buffer to gray (77 is an empirical value) */
    memset(tft_fb, 0x44, LV_HOR_RES * LV_VER_RES * sizeof(uint32_t));
#if MONITOR_DOUBLE_BUFFERED
    ...
    tft_fb_act = tft_fb;
#endif
    SDL_UpdateTexture(texture, NULL, tft_fb, LV_HOR_RES * sizeof(uint32_t));
    sdl_refr_qry = true;
    sdl_inited = true;
}
...
static void monitor_sdl_refr_core(void)
{
    if(sdl_refr_qry != false) {
        ...
        SDL_UpdateTexture(texture, NULL, tft_fb, LV_HOR_RES * sizeof(uint32_t));
#else
        SDL_UpdateTexture(texture, NULL, tft_fb_act, LV_HOR_RES * sizeof(uint32_t));
#endif
        SDL_RenderClear(renderer);
        /*Test: Draw a background to test transparent screens (LV_COLOR_SCREEN_TRANSP)*/
//        SDL_SetRenderDrawColor(renderer, 0xff, 0, 0, 0xff);
//        SDL_Rect r;
//        r.x = 0; r.y = 0; r.w = LV_HOR_RES; r.w = LV_VER_RES;
